...
public class ProceduralPotionSpawner : MonoBehaviour
{
    public Terrain terrain;  
    public GameObject[] potionPrefabs; // Plusieurs types de potions
    public int potionCount = 50;
    ...
    void SpawnPotions()
    {
        ...
        List<GameObject> spawnedPotions = new List<GameObject>();

        int attempts = 0;
        int maxAttempts = potionCount * 5; // Limite pour éviter boucle infinie

        while (spawnedPotions.Count < potionCount && attempts < maxAttempts)
        {
            attempts++;
            ...
            if (IsPositionValid(potionPosition, exclusionZones))
            {
                // Créer une potion aléatoire
                GameObject potion = Instantiate(potionPrefabs[Random.Range(0, potionPrefabs.Length)], potionPosition, Quaternion.identity);
                spawnedPotions.Add(potion);
            }
        }
    }

    // Récupérer les zones à exclure (joueurs, ennemis, arbres, etc.)
    List<Vector3> GetExclusionZones()
    {
        List<Vector3> exclusionZones = new List<Vector3>();
        string[] tagsToExclude = { "Player", "Enemy", "Arbre", "Rocher", "Plant" };
    ...
    // Vérifier si la position de la potion est valide
